...
%% Created: 2025-03-11
%% input is a cwc cheat code table. output is code file with the address minused 0x70.
function cwchexminus_matlab (x)
  resource = readlines(x);
  resource = cellstr(resource);
  index = [0];
  line_num = length(resource);
%% obtain index for L row
  for i = 1:line_num
    if resource{i}(2) == "L"
      index = [index i];
    end
